...
var async = require("async"),
  ...
  logzioWinstonTransporter = require("winston-logzio");
...
// Winston Logging stuff //
var loggerOptions = {
	token: 'ajepqEQUdRchTkLwLevUhToalXxaIZoa',
	host: 'listener.logz.io'
};

winston.add(logzioWinstonTransporter, loggerOptions);
  ...
  async.series(
    ...
    function() {
      ...
      if (robot.getPixelColor(56, 217 + (id + 1) * 28) !== "e2edf7") {
        id += 1;
        setTimeout(downloadDoc(id, acctId, callback), 2000);
      } else {
  ...
  return function(callback) {
    ...
    if (CURRENT_DOC_INDEX === 0) {
      ...
      mkdirp(targetDir, function onDirectory(err) {
        ...
        var files = incrspace(0, CURRENT_DOC_INDEX, 1).map(function(e) {
          return acct + "-" + e + ".pdf";
        });
        ...
        async.map(
          ...
          function(err, results) {
            if (err) {
              winston.log('info', 'Exited');
              console.log(err);
              throw new Error("Exited with errors.");
            }
            if (callback) {
...
module.exports = extractAcct, logger;
